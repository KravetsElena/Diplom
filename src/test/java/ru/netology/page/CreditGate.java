...
public class CreditGate {
    ...
    public void checkNotificationOk() {
        notificationOk.shouldBe(Condition.visible, Duration.ofSeconds(25)).shouldHave(exactText("Успешно Операция одобрена Банком."));
    }

    public void checkNotificationError() {
        notificationError.shouldBe(Condition.visible, Duration.ofSeconds(25)).shouldHave(exactText("Ошибка! Банк отказал в проведении операции."));
    }
